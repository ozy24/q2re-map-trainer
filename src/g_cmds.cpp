...
void Cmd_Inven_f(edict_t *ent)
{
	...
	// Map Trainer: Check if a menu is already open
	if (ent->client->menu)
	{
		PMenu_Close(ent);
	...
	// Map Trainer: Close inventory if it's open
	if (cl->showinventory)
	{
	...
	// ZOID: Handle CTF team selection
	...
	// Map Trainer: Open Map Trainer menu instead of inventory
...
void ClientCommand(edict_t *ent)
{
	...
	// Map Trainer commands
	else if (Q_strcasecmp(cmd, "maptrainer") == 0)
		Cmd_MapTrainerMenu_f(ent);
	else if (Q_strcasecmp(cmd, "savepos") == 0)
		Cmd_SetSpawn_f(ent);
